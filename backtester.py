...
class BackTester:
    ...
    def get_sharpe_ratio(self, risk_free_rate=0.0):
        ...
        # Resample intraday capital curve to end-of-day values
    ...
    def get_granular_sharpe_ratio_window(self, window_size="6ME", period="1D"):
        ...
        for window_data in data_windows:
            ...
            # print(time_index[0], time_index[-1], sum(pnls))
    ...
    def make_pnl_graph(self):
        ...
        # Per-bar percentage change 
        pct_change = (self.data['close']/self.data['open'] - 1) * 100
        pct_change = pct_change.round(2).fillna(0)
        ...
        # bar_colors = np.where((self.data['haclose_ema'] > self.data['haopen_ema']) & (self.data['adx'] > 25), 'green', 'red')
        # bar_colors = np.where((self.data['haclose_ema'] > self.data['haopen_ema']), 'green', 'red')

#         bar_colors = np.where(
#     (self.data['haclose_ema_slow'] > self.data['haopen_ema_slow']) & (self.data['adx'] > 15), 
#     'green', 
#     np.where(
#         (self.data['haclose_ema_slow'] < self.data['haopen_ema_slow']) & (self.data['adx'] > 15), 
#         'red', 
#         'gray'  # Optional: color for bars not matching any condition
#     )
# )

        # bar_colors = np.where(
        # (self.data['ha_signal']==1), 
        # 'green', 
        # np.where(
        #     (self.data['ha_signal']==-1), 
        #     'red', 
        ...
        # bar_colors_adx = np.where(self.data['adx'] > 25, 'purple', 'blue')


        # Add the bar plot with dynamic colors
        # fig.add_trace(go.Bar(
        #     x=self.data.index,
        #     y=self.data['low'],
        #     name='HA Signal',
        #     marker=dict(color=bar_colors)  # Assign colors based on the condition
        # ))
        ...
        if self.position.qty != 0:
            ...
            if 0 <= init_idx < len(self.data):
                if self.position.qty > 0:
                    ...
                    # fig.add_annotation(x=self.data.index[init_idx] + (self.data.index[final_idx] - self.data.index[init_idx]) / 2,
                    # y=self.data['high'].max(),
                    # text=f"Close Change: {price_change:.2f}",
                    # showarrow=False,
...
if __name__ == "__main__":
    ...
    # print(bt.get_granular_sharpe_ratio("1D"))
    # print(bt.get_granular_sharpe_ratio_window("6ME", "1D"))
